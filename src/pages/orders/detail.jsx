...
import { ArrowLeft, Printer, ExternalLink, Mail, Phone, MapPin, User, FileText } from "lucide-react";
...
export default function MinimalistOrderDetail() {
  ...
  const formattedDate = isValid(orderDate) ? format(orderDate, "MMMM d, yyyy 'at' h:mm a") : "—";
  ...
  return (
    <main className="flex-1 overflow-y-auto mt-10 md:mt-0  bg-slate-50/50 min-h-screen text-slate-800">
      

      <div className="hidden">
        <div ref={voucherRef} className="p-10 max-w-[80mm] mx-auto text-black font-sans space-y-6">
          <div className="text-center space-y-2 flex flex-col items-center">
            <QRCode size={75} value={fullUrl} />
            <p className="text-[8px] tracking-wider text-gray-400 uppercase mt-1">Scan to Verify Order</p>
          ...
          <div className="border-t border-b border-dashed border-gray-300 py-3 text-xs space-y-1">
            <div className="flex justify-between"><span>Customer:</span><span className="font-semibold">{order?.customer_id?.name || order?.customer?.name}</span></div>
            <div className="flex justify-between"><span>Phone:</span><span className="font-semibold">{order?.customer_id?.phone || order?.customer?.phone}</span></div>
            <div className="flex justify-between"><span>Method:</span><span className="font-semibold uppercase">{order?.payment_method}</span></div>
          </div>
          <div className="space-y-2 text-xs">
            {order?.items?.map((item, index) => (
              <div key={index} className="flex justify-between items-start">
                ...
                <p className="font-semibold">{(item?.price * item?.quantity).toLocaleString()} Ks</p>
              </div>
            ))}
          </div>
    ...
    <div className="space-y-1 min-w-0">
      <div className="flex flex-wrap items-center gap-2">
        <h1 className="text-base sm:text-lg md:text-xl font-bold text-slate-900 tracking-tight whitespace-nowrap">
          Order ID: <span className="font-mono text-slate-600 font-semibold">#{displayId}</span>
        </h1>
        <span className={`text-[10px] uppercase font-bold tracking-wide px-2.5 py-0.5 rounded-full border shrink-0 ${paymentStatusColors[currentPaymentStatus] || "bg-slate-50 text-slate-500"}`}>
  ...
  <div className="flex items-center gap-2 pl-11 md:pl-0 ">
    ...
    >
      ...
      <span>Print Invoice</span>
        ...
        {/* Core Layout Split Workspace (2/3 vs 1/3) */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 items-start">
          
          {/* Left Split Column Area */}
          <div className="lg:col-span-2 space-y-5">
            
            {/* Order Items Panel Widget */}
            <div className="bg-white border border-slate-200/80 rounded-xl overflow-hidden">
              <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/40">
                <h2 className="text-xs font-bold uppercase tracking-wider text-slate-500">Order Items</h2>
                <span className="text-xs font-medium text-slate-400">{order?.items?.length || 0} unique lines</span>
              </div>
              <div className="divide-y divide-slate-100">
                {order?.items?.map((item, idx) => (
                  <div key={idx} className="p-5 flex items-center justify-between gap-4 hover:bg-slate-50/30 transition-colors">
                    <div className="flex items-center gap-3.5 min-w-0">
                      ...
                      <div className="min-w-0">
                        <h3 className="text-sm font-semibold text-slate-900 truncate tracking-tight">
                          {item?.product_id?.name || item?.name || "Product Item"}
                        </h3>
                        <p className="text-[11px] font-medium text-slate-400 uppercase tracking-tight mt-0.5">
                          Size: <span className="text-slate-600 font-semibold">{item?.size || "—"}</span> &bull; Color: <span className="text-slate-600 font-semibold">{item?.color || "—"}</span>
                        </p>
                      </div>
                    </div>
                    <div className="text-right shrink-0 flex items-center gap-6">
                      <span className="text-xs font-medium text-slate-400 bg-slate-100/80 px-2 py-1 rounded-md">
            ...
            <div className="bg-white border border-slate-200/80 rounded-xl p-5 space-y-4">
              ...
              <div className="space-y-2.5 text-xs font-medium">
                <div className="flex justify-between text-slate-500">
                  <span>Subtotal ({order?.items?.reduce((a, b) => a + (b.quantity || 0), 0) || 0} items)</span>
                  <span className="text-slate-800 font-semibold">{order?.total_amount?.toLocaleString()} Ks</span>
                </div>
                <div className="flex justify-between text-slate-500">
          ...
          {/* Right Split Column Area */}
          <div className="space-y-5">
            
            {/* Customer Summary Mini Profile Frame */}
            <div className="bg-white border border-slate-200/80 rounded-xl p-5 space-y-3.5">
              <div className="flex items-center gap-2 text-slate-400 border-b border-slate-100 pb-2.5">
                <User className="w-4 h-4 text-slate-500" />
                <h2 className="text-xs font-bold uppercase tracking-wider text-slate-500">Customer Details</h2>
              </div>
              <div className="space-y-1">
                <p className="text-sm font-bold text-slate-900">
                  {order?.customer_id?.name || order?.customer?.name || "Guest Customer"}
                </p>
                <p className="text-[11px] font-medium text-slate-400">
                  Registered Purchase Account
                </p>
              </div>
            </div>

            {/* Direct Information Channel Block */}
            <div className="bg-white border border-slate-200/80 rounded-xl p-5 space-y-3">
              ...
              <div className="space-y-2.5 text-xs font-medium text-slate-700">
                <div className="flex items-center gap-2.5">
                  ...
                  <span>{order?.customer_id?.phone || order?.customer?.phone || "No phone recorded"}</span>
                </div>
                <div className="flex items-center gap-2.5 text-slate-400">
                  <Mail className="w-3.5 h-3.5 shrink-0" />
                  <span className="italic text-slate-400 text-[11px]">No email account associated</span>
                </div>
              </div>
            </div>

            {/* Logistics Physical Address Panel */}
            <div className="bg-white border border-slate-200/80 rounded-xl p-5 space-y-3">
              <h2 className="text-xs font-bold uppercase tracking-wider text-slate-500 border-b border-slate-100 pb-2.5">
              ...
              <div className="flex gap-2.5 text-xs font-medium text-slate-700 items-start leading-relaxed">
                ...
                <div>
                  <p className="text-slate-800 font-semibold">{order?.city || "Unknown City"}</p>
                  <p className="text-slate-500 text-[11px] mt-0.5">{order?.address || "Missing full distribution route street details"}</p>
                </div>
              </div>
            </div>

           
            <div className="bg-white border border-slate-200/80 rounded-xl p-5 flex flex-col items-center justify-center text-center space-y-2.5">
              <div className="p-2 bg-slate-50 border border-slate-100 rounded-lg">
                <QRCode size={80} value={fullUrl} fgColor="#1e293b" />
              </div>
              <div>
                <p className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Verification Endpoint</p>
                <p className="text-[11px] text-slate-500 mt-0.5 max-w-[180px]">Scan token path to verify or parse live state payload.</p>
              </div>
            </div>
